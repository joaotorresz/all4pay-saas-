...
// Recebe item/created|updated, puxa item→contas→transações da Pluggy e faz
// UPSERT idempotente em pluggy_items/bank_accounts/bank_transactions. Depois o
// ETL espelha cada transação em `movements` (reference_code='pluggy:<txId>',
// idempotente pelo índice movements_pluggy_ref_uniq) → alimenta o hub
// getRiscoInput/Fluxo/DRE. Escreve com service-role e seta org_id explícito
// (auth_org_id() não resolve fora de sessão).
...
async function etlMovements(
  ...
  for (const t of txs) {
    ...
    // insere; se já existe (23505) reaproveita o id existente
