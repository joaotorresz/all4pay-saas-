...
| **1** | Ingestão→razão: mapear transações em lançamentos; categorização; conciliação. | 🟡 **parcial** — `/razao` (GL UI: balancete + lançamentos) + `src/lib/ledger.ts` (`backfillRazao` deriva dupla entrada dos `movements`, idempotente por `external_key=mov:<id>`; `postarLancamento` manual; live insere em `journal_entries/_lines` e marca `posted`) + plano de contas (`core/ledger/chart`) com **categorização por regras** (classificador do DRE). ⏳ falta: ponte **Pluggy raw→razão** + **categorização Claude** (chave) p/ transações sem categoria. |
