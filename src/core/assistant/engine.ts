...
export function responderLocal(pergunta: string, input: RiskInput, ctx?: ExecutiveContext): (RespostaCopiloto & { contatoId?: string }) | null {
  ...
  // rouba por substring.
  if (/(maior(es)?|melhor(es)?|principa(l|is)) clientes?|quem (mais|s[ãa]o) (paga|compra|fatura|me paga|meus? (melhor|maior))|quem (me )?paga mais|qual cliente (mais )?(compr\w*|pag\w*|fatur\w*|vend\w*|gast\w*)|cliente que (mais )?(compr\w*|pag\w*)|quem (mais )?compr\w* (comigo|de mim|aqui|mais)|top clientes?|melhores clientes|(quero ver|mostra|me mostra|lista|ver) (meus |os )?clientes/.test(p) && !/representa|depend|concentra|forneced|quant(os|as)/.test(p)) {
