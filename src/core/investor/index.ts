...
/** Monta o e-mail pronto (pt-BR) — os campos do fundador entram nas seções. */
export function gerarTextoInvestorUpdate(
  u: InvestorUpdate,
  extras?: { empresa?: string; destaques?: string; pedidos?: string },
): string {
  const r = u.raw;
  const linhas: string[] = [];
  const empresa = extras?.empresa?.trim();
  linhas.push(`${empresa ? empresa + " — " : ""}Investor update · ${u.mesReferencia}`);
  linhas.push("");
  linhas.push("TL;DR");
  linhas.push(
    `Fechamos ${u.mesReferencia} com ${brl(r.receitaMes)} de receita (${r.crescimentoMoM >= 0 ? "+" : ""}${pct(r.crescimentoMoM, 1)} MoM), ` +
    `caixa de ${brl(r.caixa)} e ${r.burn > 0 ? `burn de ${brl(r.burn)}/mês (runway de ${r.runwayMeses.toLocaleString("pt-BR", { maximumFractionDigits: 1 })} meses)` : "geração de caixa positiva"}.`,
  );
  linhas.push("");
  linhas.push("Métricas");
  for (const k of u.kpis) {
    linhas.push(`- ${k.label}: ${typeof k.valor === "number" ? brl(k.valor) : k.valor}`);
  }
  ...
  if (extras?.pedidos?.trim()) {
    linhas.push("");
    linhas.push("Como vocês podem ajudar");
    linhas.push(extras.pedidos.trim());
  }
  linhas.push("");
  linhas.push(`— gerado pelo all4pay (${u.versaoModelo}) com os números reais do sistema`);
  return linhas.join("\n");
}
