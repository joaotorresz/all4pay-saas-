...
import { Card, StatusBadge, Switch, Icon, Skeleton, Select } from "@/components/ui";
        ...
        {isLoading ? <Card><Skeleton className="h-64 w-full" /></Card> : (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <Kpi label="Saldo IULI" v={formatBRL(saldoIuli)} />
              <Kpi label="Saldo OFX (extrato)" v={formatBRL(saldoOfx)} />
              ...
              <Kpi label="Conciliados / pendentes" v={`${conciliados.length} / ${naoConc.length}`} />
            </div>
            ...
            {aba === "conciliacao" && (
              <Card padded={false}>
                <div className="flex items-center gap-2 px-5 py-3 border-b border-border-soft">
                  ...
                  <span className="ml-auto text-caption text-faint">IULI × OFX · {listaFiltrada.length} transações</span>
                </div>
            ...
            {aba === "extrato" && (
              <Card padded={false}>
                <div className="px-5 py-3 border-b border-border-soft text-label font-medium text-muted">Extrato (OFX) · {contaId ? (contas.data ?? []).find((a) => a.id === contaId)?.name : "todas as contas"}</div>
            ...
            {aba === "fechamentos" && (
              <Card padded={false}>
                <div className="px-5 py-3 border-b border-border-soft text-label font-medium text-muted">Fechamentos mensais — travam o período da conta</div>
                {(() => {
