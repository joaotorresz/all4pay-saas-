          ...
          <div className="flex items-center gap-1 flex-wrap">
            {meses.map((m) => (
              <button key={m} onClick={() => setMes(m)}
                className={`text-caption font-medium rounded-pill px-3 py-1 inline-flex items-center gap-1 ${m === mesAtivo ? "bg-ink text-white" : "bg-surface-2 text-muted hover:text-ink"}`}>
                {mesLabel(m)}
